...
describe('Should fill all admin details', function(){ 
   it('Verify if click admin button',function(){
    ...
    cy.get('[id="mobile-number"]').type('6379000007')
    .should('be.visible')
    .should('be.enabled')
    .should('have.value', '6379000007')
    cy.get('[id="email-address"]').type('6379000007@mailinator.com')
    .should('be.visible')
    .should('be.enabled')
    .should('have.value', '6379000007@mailinator.com')
    cy.wait(3000)
    ...
    cy.get('[id="developerAdminId"]').should('not.be.checked').should('have.value','DEVELOPER_ADMIN')
    cy.get('button.etUmhU:nth-child(2) > div:nth-child(1)').should('have.text','Invite Admin').click({force:true})
    cy.get('button').contains('Invite Admin').click()
    cy.get('button.etUmhU:nth-child(2) > div:nth-child(1)').should('have.text','Invite Admin').click({force:true})
    ...
    cy.wait(3000)
     cy.get('button').contains('Sub Business').click()
      ...
      cy.get('[id="adminEmail"]').type('PrimaryContact123@mailinator.com')
      cy.get('[id="adminMobile"]').type('8787805656')
      cy.get('[id="gst"]').type('22AZSNE4545H1Z2')
